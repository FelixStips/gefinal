        ...
        for p in players:  # then update the players
            if p.participant.is_employer is True:
                ...
                if p.is_employed:
                    ...
                    p.effort_cost_tokens = session.config['effort_costs_points'][p.effort_choice - 1] * session.config[
                        'exchange_rate']
                    ...
                    p.participant.vars['round_for_points'].append(p.participant.vars['currency_is_points'])
        for p in players:
            print('Player', p.participant.playerID, 'has had point payoffs of', p.participant.vars['total_points'][0],
                  'and token payoffs of', p.participant.vars['total_tokens'], 'and played for',
                  p.participant.vars['round_for_points'])
